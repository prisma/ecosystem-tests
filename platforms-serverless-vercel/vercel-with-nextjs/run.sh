...
pnpm vercel deploy --prod --yes --force --token=$VERCEL_TOKEN --build-env DEBUG="prisma:*" --env DATABASE_URL=$DATABASE_URL --build-env PRISMA_CLIENT_ENGINE_TYPE="$PRISMA_CLIENT_ENGINE_TYPE" --scope=$VERCEL_ORG_ID \
| grep -oE 'https?://[^ ]+' \
| tail -n 1 \
